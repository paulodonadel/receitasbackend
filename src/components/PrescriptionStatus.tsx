...
import { CheckCircleOutline, ErrorOutline, HourglassEmpty, AssignmentTurnedIn, Email, LocalHospital, HelpOutline, ExpandMore as ExpandMoreIcon } from '@mui/icons-material';
  ...
  if (loading) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="calc(100vh - 200px)">
        ...
        <Typography variant="h6" sx={{ ml: 2 }}>Carregando suas solicitações...</Typography>
      </Box>
    );
  }
  ...
  if (prescriptions.length === 0) {
    return (
      <Container maxWidth="md">
        ...
        <Box textAlign="center" sx={{ mt: 4, p: 3, backgroundColor: alpha(theme.palette.info.light, 0.1), borderRadius: 2 }}>
            <Email sx={{ fontSize: 60, color: 'primary.main', mb: 2 }}/>
            <Typography variant="h5" gutterBottom>Nenhuma solicitação encontrada</Typography>
        ...
        {prescriptions.map((p) => {
          ...
          return (
            <Grid item xs={12} key={p.id}>
              <StatusPaper elevation={3} sx={{ borderColor: isRejected ? 'error.main' : 'primary.main' }}>
                ...
                <Collapse in={isExpanded} timeout="auto" unmountOnExit>
                    <Box sx={{ p: 2, mt: 1, borderTop: '1px dashed grey' }}>
